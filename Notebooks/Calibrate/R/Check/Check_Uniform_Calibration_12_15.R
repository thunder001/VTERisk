...
test1 = read.csv('M1_test_12_15a.csv')
test2 = read.csv('M2_test_12_15a.csv')
test3 = read.csv('M3_test_12_15a.csv')
test4 = read.csv('M4_test_12_15a.csv')
...
par(mfrow = c(1,4))
P1 = data.frame(p = test1$probs, y = ifelse(test1$golds=="True", 1, 0)  )
P2 = data.frame(p =  test2$probs, y = ifelse(test2$golds=="True", 1, 0)  )
P3 = data.frame(p =  test3$probs, y = ifelse(test3$golds=="True", 1, 0)  )
P4 = data.frame(p =  test4$probs, y = ifelse(test4$golds=="True", 1, 0)  )
...
G12 = grid.arrange(
  list(calibration_plot(data = P1, obs = 'y', pred = 'p', x_lim = c(0,.5), y_lim = c(0,.5), nTiles = 20),
  calibration_plot(data = P2, obs = 'y', pred = 'p', x_lim = c(0,.5), y_lim = c(0,.5), nTiles = 20)))
  

  
grid = grid.arrange(
calibration_plot(data = P1, obs = 'y', pred = 'p', x_lim = c(0,.5), y_lim = c(0,.5), nTiles = 20),
calibration_plot(data = P2, obs = 'y', pred = 'p', x_lim = c(0,.5), y_lim = c(0,.5), nTiles = 20),
calibration_plot(data = P3, obs = 'y', pred = 'p', x_lim = c(0,.5), y_lim = c(0,.5), nTiles = 20),
calibration_plot(data = P4, obs = 'y', pred = 'p', x_lim = c(0,.5), y_lim = c(0,.5), nTiles = 20))
