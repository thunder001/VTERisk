...
#Dev3_balanced = redo_frequency_on_dev(Pdev3, P3,d1 = 2, d2 = .5, eps = -.5)
#Dev4_balanced = redo_frequency_on_dev(Pdev4, P4,d1 = 5, d2 = -.5, eps = -1)
...
first_plot_Dev_balanced (Dev1_balanced, Dev2_balanced, Dev3_balanced, Dev4_balanced)
...
spline_pred2 = function(Ptrain, test , knots = 15, pen = .75 ){
  Ptrain$l = logit(Ptrain$p)
  test$l = logit(test$probs)
  spline = smooth.spline(x = Ptrain$l, y = Ptrain$y, #w = 1/Ptrain$Freq,
                         nknots=knots, penalty = pen  )
  spl_pred = predict(spline, test$l)
  spl_pred$y
}
...
spline_pred = function(){
    ...
    cv2 = spline_pred (Dev2_balanced, test2,  knots = 12, pen = 1.95) 
    ...
    cv3 = spline_pred (Dev3_balanced, test3) 
...
spline_pred_dev = function(){
  ...
  dCal2  = calibration_plot(data = P.cv2, obs = 'y', pred = 'p', x_lim = c(0,.05), y_lim = c(0,.05), 
                            xlab = "Binned Pred (Interval 2: 3-6 Months after Index Date)")

  cv3 = spline_pred (Dev3_balanced,  Mdevs$Y3) 
  P.cv3 =  data.frame(p = as.numeric(cv3), y = ifelse( Mdevs$Y3$golds=="True", 1, 0)  )
  dCal3  = calibration_plot(data = P.cv3, obs = 'y', pred = 'p', x_lim = c(0,.05), y_lim = c(0,.05),
                            xlab = "Binned Pred (Interval 3: 6-9 Months after Index Date)")
  
  cv4 = spline_pred (Dev4_balanced, Mdevs$Y4, knots = 29, pen = .01) 
  P.cv4 =  data.frame(p = as.numeric(cv4), y = ifelse(Mdevs$Y4$golds=="True", 1, 0)  )
  dCal4  = calibration_plot(data = P.cv4, obs = 'y', pred = 'p', x_lim = c(0,.05), y_lim = c(0,.05), 
                            xlab = "Binned Pred (Interval 4: 9-12 Months after Index Date)")
...
spline_traindev_for_calibration = function(){
      
  make_Reps_train = function(P){
    Reps = data.frame( table( P$pid ) )
    Reps$pid = as.character(Reps$Var1) %>%as.integer()
    P.rep = join(P, Reps)
    P.rep$Var1 = NULL
  ...
  PP1 = make_Reps_train(P1)
  
  TrD1 = rbind(make_Reps_train(P1), Dev1_balanced)
  TrD2 = rbind(make_Reps_train(P2), Dev2_balanced)
  TrD3 = rbind(make_Reps_train(P3), Dev3_balanced)
  TrD4 = rbind(make_Reps_train(P4), Dev4_balanced)
  
  cv1 = spline_pred (TrD1, test1,  knots = 4, pen = 1) 
  P.cv1 =  data.frame(p = as.numeric(cv1), y = ifelse(test1$golds=="True", 1, 0)  )
  tCal1  = calibration_plot(data = P.cv1, obs = 'y', pred = 'p', #x_lim = c(0,.1), y_lim = c(0,.1), 
                            xlab = "Binned Pred (Interval 1: 0-3 Months after Index Date)", nTiles =10)
  ...
  cv2 = spline_pred (TrD2, test2,  knots = 12, pen = 1.95) 
  P.cv2 =  data.frame(p = as.numeric(cv2), y = ifelse(test2$golds=="True", 1, 0)  )
  tCal2  = calibration_plot(data = P.cv2, obs = 'y', pred = 'p', #x_lim = c(0,.05), y_lim = c(0,.05), 
                            xlab = "Binned Pred (Interval 2: 3-6 Months after Index Date)")
  tCal2
  
  cv3 = spline_pred (TrD3, test3) 
  P.cv3 =  data.frame(p = as.numeric(cv3), y = ifelse(test3$golds=="True", 1, 0)  )
  tCal3  = calibration_plot(data = P.cv3, obs = 'y', pred = 'p', x_lim = c(0,.05), y_lim = c(0,.05),
                            xlab = "Binned Pred (Interval 3: 6-9 Months after Index Date)")
  tCal3
  
  cv4 = spline_pred (TrD3, test4, knots = 29, pen = .01) 
  P.cv4 =  data.frame(p = as.numeric(cv4), y = ifelse(test4$golds=="True", 1, 0)  )
  tCal4  = calibration_plot(data = P.cv4, obs = 'y', pred = 'p', x_lim = c(0,.05), y_lim = c(0,.05), 
                            xlab = "Binned Pred (Interval 4: 9-12 Months after Index Date)")
  #tCal4
...
temperature_traindev_for_calibration = function(){
  ...
  cv_logit = function(Ptrain, test ){
    train_cv = cv.glmnet(x = cbind(1  , logit(Ptrain$p) ),family = 'binomial', y = Ptrain$y, 
                         type.measure = 'mae', weights = 1/Ptrain$Freq, intercept = FALSE)
  ...
  cv2 = cv_logit (TrD2, test2 ) 
  P.cv2 =  data.frame(p = as.numeric(cv2), y = ifelse(test2$golds=="True", 1, 0)  )
  tCal2  = calibration_plot(data = P.cv2, obs = 'y', pred = 'p', #x_lim = c(0,.05), y_lim = c(0,.05), 
                            xlab = "Binned Pred (Interval 2: 3-6 Months after Index Date)")
  tCal2
  
  cv3 = cv_logit (TrD3, test3) 
  P.cv3 =  data.frame(p = as.numeric(cv3), y = ifelse(test3$golds=="True", 1, 0)  )
  tCal3  = calibration_plot(data = P.cv3, obs = 'y', pred = 'p', x_lim = c(0,.05), y_lim = c(0,.05),
                            xlab = "Binned Pred (Interval 3: 6-9 Months after Index Date)")
  tCal3
  
  cv4 = cv_logit (TrD3, test4) 
  P.cv4 =  data.frame(p = as.numeric(cv4), y = ifelse(test4$golds=="True", 1, 0)  )
  tCal4  = calibration_plot(data = P.cv4, obs = 'y', pred = 'p', x_lim = c(0,.05), y_lim = c(0,.05), 
                            xlab = "Binned Pred (Interval 4: 9-12 Months after Index Date)")
  #tCal4
...
spline_pred_dev_on_balancedev = function(){
  
  cv1 = spline_pred2 (Dev1_balanced,Dev1_balanced) 
  P.cv1 =  data.frame(p = as.numeric(cv1), y =  Dev1_balanced$y  )
  dCal1  = calibration_plot(data = P.cv1, obs = 'y', pred = 'p', #x_lim = c(0,.1), y_lim = c(0,.1), 
                            xlab = "Binned Pred (Interval 1: 0-3 Months after Index Date)", nTiles =10)
...
temperature_dev_for_calibration = function(){
  ...
  cv_logit = function(Ptrain, test ){
    train_cv = cv.glmnet(x = cbind(1  , logit(Ptrain$p) ),family = 'binomial', y = Ptrain$y, 
                         type.measure = 'mae', weights = 1/Ptrain$Freq, intercept=F)
    cv_pred = predict(train_cv, newx = cbind(1, logit(test$probs)), type = 'response')
    cv_pred
  }
