...
joining_weights = function(){
    ...
    Y12 = join( select( devs$Y1, X, p1 = p, y1=y) , select( devs$Y2, X,p2=p, y2 =y ) , by = 'X')
    Y123 = join( Y12, select( devs$Y3, X, p3=p, y3=y), by = 'X')
    Z = join( Y123, select( devs$Y4, X, p4=p, y4=y), by = 'X')
    ...
    Yt12 = join( select( tests$Y1, X, p1 = p, y1=y) , select( tests$Y2, X,p2=p, y2 =y ) , by = 'X')
    Yt123 = join( Yt12, select( tests$Y3, X, p3=p, y3=y), by = 'X')
    Zt = join( Yt123, select( tests$Y4, X, p4=p, y4=y), by = 'X')
    
    Ys = select(Zt, y1, y2, y3, y4)
    sum( colSums( Ys, na.rm = T) )
    
    954 / 16132
    Zt$y1 + Zt$y2 + Zt$y3 + Zt$y4
...
#train4_cv = cv.glmnet(x = cbind(1,devs$Y4$l ) , y =devs 
                      
only_4_fromZ = function(Z){
  ...
  wt4 = ifelse(devs$Y4$y ==1,  wt4/ mean(Z4$y4), wt4)
  hist(wt4)
  
  train4_cv = cv.glmnet(x = cbind(1, devs$Y4$l) , y =devs$Y4$y , weights = (wt4), family = 'binomial',
                        type.measure = 'deviance')
  
  #train4_cv = glm(x = cbind(1,devs$Y4$l ) , y =devs$Y4$y , weights = (wt4))
...
only_4_fromZ_Multi = function(Z){
  ...
  x_1234 =  cbind(1, logit(Z4$p1), logit(Z4$p2),logit(Z4$p3),logit(Z4$p4)  )
  
  train4_cv = cv.glmnet(x =  x_1234, y =devs$Y4$y , weights = (wt4), family = 'binomial',
                        type.measure = 'deviance')
  ...
  pred_1234 = cbind(1,logit(Zt4$p1), logit(Zt4$p2),logit(Zt4$p3),logit(Zt4$p4)  )
  
  cv_pred4 = predict(train4_cv, newx = pred_1234, type = 'response')
  Pred4 = data.frame(p = cv_pred4[,1], y = tests$Y4$y)
  calibration_plot(data = Pred4, obs = 'y', pred = 'p', x_lim = c(0,1), y_lim = c(0,.1))
