...
class BTH_Disease_Progression_Dataset(data.Dataset):
    ...
    def get_trajectory(self, patient):
        ...
        if self.args.staggered:
            
            start_i =  [find_closest_date_before( patient['dx_date']  - datetime.timedelta(360)  , ev_dates1), 
                        find_closest_date_before( patient['dx_date']  - datetime.timedelta(270)  , ev_dates1),
                        find_closest_date_before( patient['dx_date']  - datetime.timedelta(180)  , ev_dates1),
                        find_closest_date_before( patient['dx_date']  - datetime.timedelta(90)  , ev_dates1),]
            
            ts =  [t1,t2,t3,t4]
            valid =  [  i > start_i[i] for i in range(len(start_i))]
            ...
            if idx is not None:
                ...
                if len(events_to_date)> self.args.min_events_length:
                    ...
                    if self.args.dxseq_event: 
                        ...
                        dx, dx_seq = self.get_time_seq(events_to_date,  events_to_date[0]['admit_date']  )  
