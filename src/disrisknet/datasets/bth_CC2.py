...
class BTH_Disease_Progression_Dataset(data.Dataset):
    ...
    def get_trajectory(self, patient):
        ...
        if self.args.sensitivity:
            lookback = patient['index_date'] + datetime.timedelta(30)  
        
        
        if self.args.sensitivity60:
            lookback = patient['index_date'] + datetime.timedelta(60)  
        
        if self.args.sensitivity_dx60:
            lookback = patient['dx_date'] + datetime.timedelta(60)  
                       
         
        if self.args.sensitivity_dx200:
            lookback = patient['dx_date'] - datetime.timedelta(200)  
        
        
        if self.args.sensitivity_dx365:
            lookback = patient['dx_date'] - datetime.timedelta(365)  
                              
        if self.args.sensitivity_dx_median:
            lookback = patient['dx_date'] + datetime.timedelta(180)  
            
        elif self.args.looking_backward: 
            if patient['outcome']:

                lookback = patient['outcome_date'] - datetime.timedelta(self.args.days)
                time_from_index = lookback - patient['index_date']
               
                if self.args.case_noise_fixed:
                    case_noise = random.choice(np.arange(0, 30)) # for fixed case noise
                
                if self.args.case_noise:
                    days_from_index = int(time_from_index.days)    
                    if days_from_index>0:
                        d = days_from_index
                    else:
                        d = 30                    
                    case_noise = random.choice(np.arange(0,  d))                        
                lookback = lookback - datetime.timedelta(int(case_noise) )        

                #if lookback < patient['dx_date']:
                #    lookback = patient['dx_date']
            
            # controls
            else:
                if self.args.med_from_index: 
                    expected_date = 240 + random.choice(np.arange(-240,240))
                    lookback = patient['index_date'] + datetime.timedelta(int(expected_date) )      
                elif self.args.adj_med_from_index:
                    expected_date = 240 - self.args.days 
                    expected_date= expected_date + random.choice(np.arange(-60,60))
                    lookback = patient['index_date'] + datetime.timedelta(int(expected_date) )      
                elif self.args.any_control0:
                    expected_date=  random.choice(np.arange(0,680))
                    expected_date_index = np.random.binomial(1,.5) * expected_date
                    lookback = patient['index_date'] + datetime.timedelta(int(expected_date_index) )
                elif self.args.ctrl_fromdx:
                    expected_date=  np.random.exponential(scale=100, size=1)
                    if expected_date > 730:
                        expected_date=random.choice(np.arange(0,730))
                    lookback = patient['dx_date'] + datetime.timedelta(int(expected_date) )      
                
                elif self.args.ctrl_fromdx200:
                    expected_date=  np.random.exponential(scale=100, size=1)
                    if expected_date > 730:
                        expected_date=random.choice(np.arange(0,730))
                    lookback = patient['dx_date']-  datetime.timedelta(int(200) )  + datetime.timedelta(int(expected_date) )      
                else:
                    lookback = patient['index_date'] 
        ...
        for idx in selected_idx:
            ...
            elif self.args.dxseq_event2:
                dx, dx_seq = self.get_event_seq(  patient['dx_date'], events_to_date[-1]['admit_date'] )   
            else:
    ...
    def __getitem__(self, index):
        ...
        for sample in samples:
            ...
            code_str = " ".join(codes)    # code_str = np.array(code_str).astype(np.string_)
