...
if __name__ == '__main__':
    ...
    # to combine Phecode (with dx date and BMI added) and Basic Labs + CMP (14 labs)

    VTE_path = 'Y:\\Fillmore_Cancer\\cat\\Users\\data\\derived\\vte_ml\\vte_ml\\data_for_chunlei'
    phe_path = os.path.join(VTE_path, 'Augment\\Xtot.json')
    CMP_path = os.path.join(VTE_path, 'Lab_Transfer\\labCMPtot2.json')
    phe_CMP =  combine_icd_drug (phe_path, CMP_path )
    combined_path = os.path.join(VTE_path, 'A2_CMP\\XPheLab_CMP.json')
    json.dump(phe_CMP, open(combined_path, 'w'))
    train_dev_test_file_random_split(combined_path, train_size=0.6, dev_size=0.2, test_size=0.2)